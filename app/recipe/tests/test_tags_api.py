...
class PrivateTagsApiTest(TestCase):
    ...
    def test_retriving_tags(self):
        """Test to retreive the list of tags"""
    ...
    def test_filter_tags_asigned_to_recipe(self):
        """Test listing tags assigned to recipes."""
        tag1=Tag.objects.create(user=self.user,name='Snacks')
        tag2=Tag.objects.create(user=self.user,name='fried')
        recipe= Recipe.objects.create(
            user=self.user,
            title='Green egg Salad',
            price=Decimal('3.33'),
            time_minutes=20,
        )
        recipe.tag.add(tag1)
